...
const dropTarget = {
  ...
  drop(props, monitor, component) {
    ...
    const { updateCardStatus, status_id } = props
    updateCardStatus(id, status_id, project_id);
    ...
    return connectDropTarget(
      <div className='card-wall'>
        <div className='card-wall-wrapper'>
          ...
          <div style={{ maxHeight: 'calc(100vh - 232px)' }} className='card-wall-content'>
            {children}
            {isOver && canDrop && <Card data={tempMonitor.data} id={tempMonitor.id} status={tempMonitor.status} />}
          </div>
        </div>
      </div>
    )
