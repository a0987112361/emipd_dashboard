...
class Activity3Screen extends React.Component {
  constructor(props) {
    super(props);
  ...
  handleChangeContent = (text, value) => {
    if (text == "activityContentZh") {
      this.setState({ activityContentZh: value });
    } else if (text == "activityContentEn") {
      this.setState({ activityContentEn: value });
    }
  };
  ...
  beforeFilmUpload = (file) => {
    ...
    video.onloadedmetadata = (evt) => {
      if (isLoading && isOk) this.setState({ filmPath: file });
      return isOk;
    };
  };
    ...
    if (bool) {
      this.getBase64(
        ...
        () =>
          this.setState({
            loading: false,
            filmPath: info.file.originFileObj,
            is_film: true,
          }),
        "film"
      );
  ...
  handleSubmit = (value) => {
    ...
    formData.append(
      "activity_releasetime",
      moment(value.activity_releasetime).format("YYYY-MM-DD")
    );
    ...
    if (mode === "update") {
      ...
      if (filmPath.length === 0) {
        // 如果原本有影片但刪掉 給video_id空字串
        formData.append("video_id", "");
      } else if (_.has(filmPath, "uid")) {
        // 如果有重新上傳的動作
      ...
      if (moreImageData.length < 0) {
        // 如果把原來有的照片「全部刪除」，就把原本的照片id都推進去delete_imgs陣列裡
        ...
        let newTemp = [];
        let delTemp = [];
        originFileData.map((item) => {
          oldTemp.push(item.id);
        });
          ...
          initialValues={
            mode === "update"
              ? {
                  ...
                  activity_releasetime: moment(
                    activityInfo.activity_releasetime
                  ),
                }
              : {}
          }
        ...
        >
          ...
          <div style={{ marginTop: "16px" }}>
            ...
            <HtmlEditor
              ...
              onEditorStateChange={(value) =>
                this.handleChangeContent("activityContentZh", value)
              }
            />
          </div>
          ...
          <div style={{ marginTop: "16px" }}>
            ...
            <HtmlEditor
              ...
              placeholder="請輸英文內文"
              onEditorStateChange={(value) =>
                this.handleChangeContent("activityContentEn", value)
              }
            />
          </div>
    ...
    const columns = [
      {
        ...
        render: (value, record) => (
          <Switch
            checked={value}
            onChange={() => this.handleChangePin(record)}
          />
        ),
      },
      {
          ...
          return (
            <div>
              <Button
                ...
                onClick={() =>
                  this.handleViewModal("update", record.activity_id)
                }
              >
                <img src={Images.edit} />
              </Button>
...
export default connect(
  (state) => ({
    activityList: state.activity3.activityList,
    paging: state.activity3.paging,
    activityPicList: state.activity3.activityPicList,
    picPaging: state.activity3.picPaging,
    activityInfo: state.activity3.activityInfo,
    screenHeight: state.screen.screenHeight,
  }),
  (dispatch) =>
    bindActionCreators(
      {
        getActivityList: Activity3Actions.getActivity3List,
        getActivityPicList: Activity3Actions.getActivity3PicList,
        getActivityInfo: Activity3Actions.getActivity3Info,
        createActivity: Activity3Actions.createActivity3,
        updateActivity: Activity3Actions.updateActivity3,
        deleteActivity: Activity3Actions.deleteActivity3,
        updatePin: Activity3Actions.updatePin3,
      },
      dispatch
    )
)(Activity3Screen);
