...
class Course2Screen extends React.Component {
  constructor(props) {
    super(props);
  ...
  handleRouteChange = () => {
    ...
    this.setState({
      ...
      history.push(`/course2?${queryPayload.search !== '' ? `search=${queryPayload.search}&` : ''}${queryPayload.now_page !== 1 ? `now_page=${queryPayload.now_page}&` : ''}${queryPayload.page_size !== 10 ? `page_size=${queryPayload.page_size}&` : ''}${queryPayload.order !== 0 ? `order=${queryPayload.order}` : ''}`);
    })
  }
...
export default connect(
  (state) => ({
    class: state.class,
    courseList: state.course2.courseList,
    paging: state.course2.paging,
    courseInfo: state.course2.courseInfo,
  ...
  (dispatch) =>
    bindActionCreators(
      {
        getCourseList: Course2Actions.getCourse2List,
        getCourseInfo: Course2Actions.getCourse2Info,
        createCourse: Course2Actions.createCourse2,
        updateCourse: Course2Actions.updateCourse2,
        deleteCourse: Course2Actions.deleteCourse2,
      },
      dispatch,
    ),
)(Course2Screen);
