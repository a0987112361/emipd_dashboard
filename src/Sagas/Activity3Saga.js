...
export function* getActivity3List({ payload, callback, errorCallback }) {
  try {
    ...
    const { data: res } = yield call(
      ...
      Activity3.getActivity3List(payload.now_page, payload.page_size, payload.search, payload.order),
    )

    if (res.success) {
      yield put(Activity3Actions.getActivity3ListSuccess(res.data.list, res.paging));
...
export function* getActivity3PicList({ payload, callback, errorCallback }) {
  try {
    ...
    const { data: res } = yield call(
      Handler.get({ Authorization: token }),
      Activity3.getActivity3PicList(payload.now_page, payload.page_size, payload.id),
    )

    if (res.success) {
      yield put(Activity3Actions.getActivity3PicListSuccess(res.data.list, res.paging));
      if (callback) { callback(res.data) }
    }
  } catch (err) {
    console.log('err', err);
...
export function* createActivity3({ payload, queryPayload, callback, errorCallback }) {
  try {
    const token = yield select((state) => state.user.Token);
    const { data: res } = yield call(
    ...
    if (res.success) {
      yield put(Activity3Actions.getActivity3List(queryPayload, callback, errorCallback));
...
export function* updateActivity3({ payload, queryPayload, callback, errorCallback }) {
  try {
    const token = yield select((state) => state.user.Token);
    const { data: res } = yield call(
...
export function* deleteActivity3({ payload, queryPayload, callback, errorCallback }) {
  try {
    const token = yield select((state) => state.user.Token);
    const { data: res } = yield call(
