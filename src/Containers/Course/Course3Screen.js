...
class Course3Screen extends React.Component {
  constructor(props) {
    super(props);
  ...
  handleRouteChange = () => {
    ...
    this.setState({
      ...
      history.push(`/course3?${queryPayload.search !== '' ? `search=${queryPayload.search}&` : ''}${queryPayload.now_page !== 1 ? `now_page=${queryPayload.now_page}&` : ''}${queryPayload.page_size !== 10 ? `page_size=${queryPayload.page_size}&` : ''}${queryPayload.order !== 0 ? `order=${queryPayload.order}` : ''}`);
    })
  }
  ...
  handleChangeContent = (text, value) => {
    if (text == 'courseContentZh'){
      this.setState({ courseContentZh: value.toHTML() });
    }else if (text == 'courseContentEn'){
      this.setState({ courseContentEn: value.toHTML() });
    }
  }
  ...
  handleSubmit = (value) => {
    ...
    const { mainImageData, mainEnImageData, queryPayload, courseContentEn, courseContentZh, mode, currentId, mainImageId, mainEnImageId, courseRegtimeString } = this.state;
    let isOk = true;
    let formData = new FormData();

    formData.append('course_regtime', '~');
    formData.append('course_releasetime', moment(value.course_releasetime).format('YYYY-MM-DD'));
    ...
    if(mode === 'update'){
      ...
      if(mainEnImageData.length === 0){ // 如果把圖片清空
        formData.append('file_id_en', '');
      }else if(_.has(mainEnImageData[0].originFileObj, 'uid')){ // 如果有重新上傳的動作
  ...
  renderViewModal = () => {
    ...
    const { viewModalVisible, courseRegtime, mainImageData, mainEnImageData, courseContentZh, courseContentEn, mode, loading, isLoading } = this.state;
        ...
        >
          <Row style={{margin: '0', rowGap: '12px'}} gutter={8}>
            ...
            <Col span={8}>
              ...
              >
                <DatePicker placeholder='請選擇發佈日期' style={styles.datePickerStyle} format="YYYY/MM/DD"/>
              </Form.Item>
            </Col>
          </Row>
          ...
          <div style={{marginTop: '16px'}}>
            ...
            <HtmlEditor
              ...
              placeholder='請輸入中文內文'
              onEditorStateChange={(value) => this.handleChangeContent('courseContentZh', value)}
            />
          </div>
          ...
          <div style={{marginTop: '16px'}}>
            ...
            <HtmlEditor
              ...
              placeholder='請輸英文內文'
              onEditorStateChange={(value) => this.handleChangeContent('courseContentEn', value)}
            />
          </div>
            ...
            >
              ...
              >
                {mainEnImageData.imgUrl ?
                  <img src={mainEnImageData.imgUrl} alt="avatar" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                  :
                  <div>
                    {loading ? <LoadingOutlined /> : <img src={Images.outline} alt="avatar" />}
                    <div style={{ marginTop: 8 }}>上傳照片</div>
...
export default connect(
  (state) => ({
    class: state.class,
    courseList: state.course3.courseList,
    paging: state.course3.paging,
    courseInfo: state.course3.courseInfo,
  ...
  (dispatch) =>
    bindActionCreators(
      {
        getCourseList: Course3Actions.getCourse3List,
        getCourseInfo: Course3Actions.getCourse3Info,
        createCourse: Course3Actions.createCourse3,
        updateCourse: Course3Actions.updateCourse3,
        deleteCourse: Course3Actions.deleteCourse3,
      },
      dispatch,
    ),
)(Course3Screen);
