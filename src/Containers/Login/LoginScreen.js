        ...
        if (record === null) {
          ...
          if (oldCount >= 4) {
            ...
            if (
              ...
              new Date(oldRecord.ok_date).getDate()
            ) {
              // 如果跟之前允許登入的日期同天
              newRecord.ok_date = next.toString();
              if (newRecord.count === 5) {
                ...
                if (
                  ...
                  new Date(oldRecord.ok_date).getTime()
                ) {
                  newRecord.count = 1;
                  this.setState({ failTimes: 1 }, () => {
                    window.localStorage.setItem(
            ...
            // 登入錯誤次數2~4次
            if (new Date(Date.now()).getTime() - new Date().getTime() == 0) {
              // 目前登入時間未超過今日12:00
              this.setState({ failTimes: this.state.failTimes + 1 }, () => {
                window.localStorage.setItem(
                  "EMIPD_FAIL",
                  JSON.stringify(newRecord)
                );
              ...
              rules={[
                { required: true, message: "請輸入新密碼" },
                {
                  validator: function (_, value) {
                    if (!value) {
                      return Promise.reject("請輸入新密碼");
                    }

                    if (value.length < 10) {
                      return Promise.reject("密碼長度需至少 10 碼");
                    }
                    ...
                    if (!hasUppercase || !hasLowercase || !hasNumber) {
                      return Promise.reject("密碼需包含英文大小寫與數字");
                    }
